 ...
 *     the lengths of the two input strings str1 and str2.
 *@TimeComplexity
 *     <p>O(min(m,n)⋅(m+n))O(\min(m, n) \cdot (m + n))O(min(m,n)⋅(m+n)) We checked every prefix string
 *     base of the shorter string among str1 and str2, and verify if both strings are made by
 *     multiples of base.
 *     <p>There are up to min(m,n)\min(m, n)min(m,n) prefix strings to verify and
 *     each check involves iterating over the two input strings to check if the current base is the
 *     GCD string, which costs O(m+n)O(m + n)O(m+n).
 *     <p>Therefore, the overall time complexity is
 *     O(min(m,n)⋅(m+n))O(\min(m, n) \cdot (m + n))O(min(m,n)⋅(m+n)).
 *@SpaceComplexity
 *     O(min(m,n))O(\min(m, n))O(min(m,n)) We need to keep a copy of base in each iteration, which
 *     takes O(min(m,n))O(\min(m, n))O(min(m,n)) space.
 * @since 02/07/23
 */
